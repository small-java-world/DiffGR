...
右ペインで「選択グループの差分を一覧表示」に切り替えるには `d` キーを使います（もう一度 `d` で通常表示に戻る）。
...
- Codex / Claude の対話画面側で `exit` / `quit` 等（CLIの案内に従う）
- うまく抜けられなければ `Ctrl+C`（このスクリプトは `Ctrl+C` 終了も正常扱いして続行します）
...
### 3.8 HTML差分レポートを出力する（WinMerge風）

選択した仮想PR（グループ）だけを、ファイル単位のHTML差分レポートとして出力できます。

```powershell
python scripts\export_diffgr_html.py `
  --input samples/diffgr/ts20-5pr.named.diffgr.json `
  --group "計算倍率変更" `
  --output out/reports/keisan-bairitsu.html `
  --open
```

`--group` は **グループID（例: `g-pr01`）または日本語名（完全一致）** を指定できます。  
`--group all` で全体レポートも出せます。
...
- フィルタ
  - `/`: フィルタ入力（ファイル名部分一致など）
- 表示モード
  - `d`: 右ペインを「単一チャンク詳細」⇔「選択グループ差分一覧」に切り替え
- 終了
  - `q`: 終了
