...
このドキュメントは、現行実装（2026-02-23 時点）に合わせた運用マニュアルです。  
...
5. 必要に応じて `Review Handoff`（`groupBriefs`）で変更者からレビュアーへ文脈を渡す
...
- review state: `reviews` に加えて `groupBriefs` も同一 JSON に保存される
...
python scripts\export_diffgr_html.py `
  ...
  --impact-old samples/diffgr/ts20-5pr.refined.diffgr.json `
...
- `scripts/extract_diffgr_state.py`: review state（`reviews / groupBriefs / analysisState / threadState`）だけを抽出
- `scripts/diff_diffgr_state.py`: 2つの review state JSON の差分（added/removed/changed）を表示
- `scripts/apply_diffgr_state_diff.py`: 2つの review state JSON の差分から選択 token だけを適用
...
- `scripts/summarize_diffgr_state.py`: review state JSON の件数・status 分布・UI state を要約
- `scripts/merge_diffgr_state.py`: 複数の review state JSON を deterministic に統合。`--preview` なら `--output` なしで summary だけ確認可能
- `scripts/preview_rebased_merge.py`: old/new DiffGR と review state JSON から impact-aware merge preview を表示
- `scripts/apply_diffgr_state.py`: 抽出済み state を別の DiffGR JSON に再適用
- `scripts/rebase_diffgr_state.py`: review state JSON を old/new DiffGR 間で引き継ぐ
...
```powershell
python scripts\view_diffgr_app.py samples/diffgr/ts20-5pr.ai-refined.diffgr.json --state out/state/review-state.json
```

外部 state を重ねずに開く場合は `--state` を省略します。
...
- `u`: 選択チャンクを未割り当て化
- `b`: 現在の group の `Review Handoff` を編集
- `Shift+B`: 現在の group の `Review Handoff` status を `draft -> ready -> acknowledged -> stale` で切替
- `Space`: done/undone（`reviewed` <-> `unreviewed`）
- `Shift+Space`: done（`reviewed`）
- `Backspace` または `1`: undone（`unreviewed`）
- `m`: コメント編集（行フォーカス中は行アンカーコメント）
- `o`: 実ファイルを外部エディタで開く
- `t`: 設定（`editor mode` / `diff syntax` / `ui density`）
- `d`: 右ペイン「通常表示」/「グループ差分一覧」切替
- `v`: チャンク詳細表示（compact / old-new side-by-side）
- `z`: Focus Changes（未変更行＝context を隠して変更点に集中。コメント付きcontextは残す）
- `Shift+T`: コードのシンタックステーマ切替（Pygments）
- `Shift+H`: review state JSON を `out/state/*.state.json` へエクスポート
- `Shift+I`: review state JSON を読み込んで現在の doc に適用
- `Ctrl+B`: review state JSON を bind
...
- `3`: `needsReReview`
- `4`: `ignored`
- `Shift+Up` / `Shift+Down`（または `j` / `k`）: 複数選択（範囲選択の拡張）
- `x`: 選択のON/OFF（現在行をトグル）
- `Ctrl+A`: 表示中の全チャンクを選択
- `Esc`: 複数選択をクリア
- `[` / `]`: 左右ペイン比率変更
- `Ctrl+Left` / `Ctrl+Right`: 左右ペイン比率変更（同上）
- `Alt+Left` / `Alt+Right`: 右ペイン内 old/new 幅比率変更
- `s`: JSON保存（初回に `.bak` 作成）
- `h`: HTMLエクスポート
- `=` / `-`: ズーム相当（ui density 切替: compact/normal/comfortable）

補足:

- Textual UI の「フォントサイズ」自体は **端末側設定**です（アプリからは変更できません）。  
  見た目を詰める/広げる用途は `ui density`（セル余白）を使います。
- シンタックステーマは `t` Settings の `diff syntax theme` または `Shift+T` で変更できます。
...
### 6.3 `Review Handoff` の仕様

- `b`: 現在の group に対する handoff を編集
- Textual では以下を編集できる
  - `summary`
  - `focusPoints`
  - `testEvidence`
  ...
  - `status`
- `Shift+B` は status だけを素早く切り替えるショートカット
- handoff は chunk comment ではなく `groupBriefs[groupId]` に保存される
- chunk をまたがる説明や reviewer への依頼事項は、chunk comment ではなく handoff に入れる
...
- `Edit Handoff` で group-level `Review Handoff` を編集
- `Download JSON` で `draftDoc` 全体を書き出し
- `Download State` で `reviews / groupBriefs / analysisState / threadState` だけを書き出し
- `Import State` で state JSON を読み込んで現在の HTML に反映
- `Copy State` で `reviews / groupBriefs / analysisState / threadState` をまとめてコピー
- `Impact Preview` は `--impact-old` と `--impact-state` を両方付けたときだけ表示
...
`Save State` ボタンで `*.diffgr.json` の `reviews / groupBriefs / analysisState / threadState` を直接更新します。`Download State` / `Copy State` / `Import State` / `Diff State` / `Apply Selected State` は同じ state payload を扱い、toolbar の state source には `embedded` / `overlay:<file>` / `imported:<file>` が表示されます。`Diff State` / `Apply Selected State` / `Impact Preview > State Diff` は同じ section order と count semantics を使い、`threadState.__files:*` も file-level token のまま表示されます。`Apply Selected State` は token を貼ったあとに `Preview` で `selected / no-op / changed-sections / State Diff` を確認してから `Apply` する 2 段階です。`--impact-old` と `--impact-state` を両方付けた場合だけ、read-only の `Impact Preview` で changed groups / affected briefs / warning kinds を確認できます。HTML の `Impact Preview` は Python core の preview report をそのまま表示し、source 行には `old -> new using state` が出ます。`Apply These Tokens` は current state source がその impact state と一致するときだけ有効で、plan row から直接 preview/apply modal に進み、その preview には `source=impact:<plan>` と `base=<imported state>` が表示されます。Textual では `Ctrl+Shift+D` の後に `Ctrl+Shift+M` で `@handoffs / @reviews / @ui / @all` を使えます。`Ctrl+Alt+M` なら plan 名だけで direct impact apply preview/apply に進めます。`@plan` は impact preview の rebased state を参照するため、明示 token と混在させません。

- `analysisState`
  - file/content/comment filter、`inbox` / `hide context` / `comments only`、wrap/compact、diff font、最後に見ていた chunk などの UI state
- `threadState`
  - file section / chunk の open/close 状態
...
- `Save State` で `/api/state` へPOST
- `--state` なし: サーバが元JSONの `reviews / groupBriefs / analysisState / threadState` を更新して保存
- `--state` あり: サーバが外部 state JSON を更新して保存
...
### 7.4 `Review Handoff` の使い分け

- `reviews[chunkId].comment`
  - chunk 単位の指摘、観察、メモ
- `reviews[chunkId].lineComments`
  - 行アンカーに結びつく指摘
- `groupBriefs[groupId]`
  - group 全体の目的、注目点、テスト証跡、既知 tradeoff、レビュアーへの質問

迷ったら:

- 「この group 全体の話」なら `Review Handoff`
- 「この chunk の話」なら chunk comment
- 「この行の話」なら line comment
...
python scripts\export_diffgr_html.py `
  ...
  --save-state-url http://127.0.0.1:8765/api/state `
  --save-state-label "Save State"
```

この場合、静的ファイルでも `Save State` ボタンが表示されます（エンドポイントが到達可能なときのみ成功）。

### 7.5 state の抽出と再適用

review state だけを別ファイルで持ち回したい場合:

```powershell
python scripts\extract_diffgr_state.py `
  --input samples/diffgr/ts20-5pr.ai-refined.diffgr.json `
  --output out/state/review-state.json
```

抽出した state を別の DiffGR JSON に戻したい場合:

```powershell
python scripts\apply_diffgr_state.py `
  --input out/work/r2.review.diffgr.json `
  --state out/state/review-state.json `
  --output out/work/r2.review.with-state.diffgr.json
```

用途:

- HTML の `Copy State` / `Download State` で取った state を CLI で再適用する
- HTML に戻したいときは `Import State` で state JSON を再読込できる
- 別 repo で管理している review state を bundle 本体へ戻す
- `reviews` だけでなく `groupBriefs / analysisState / threadState` もまとめて持ち運ぶ

state JSON の中身を手早く確認したい場合:

```powershell
python scripts\summarize_diffgr_state.py `
  --input out/state/review-state.json
```

2つの state の差分を確認したい場合:

```powershell
python scripts\diff_diffgr_state.py `
  --base out/state/reviewer-a.json `
  --other out/state/reviewer-b.json
```

state を複数人分まとめたい場合:

```powershell
python scripts\merge_diffgr_state.py `
  --base out/state/base.json `
  --input out/state/reviewer-a.json `
  --input out/state/reviewer-b.json `
  --output out/state/merged.json
```
...
- `reviews`: status precedence と line comment append-merge
- `groupBriefs`: field-wise merge
- `analysisState`: 後勝ち
- `threadState`: `__files` は部分 merge、それ以外は後勝ち

state だけを新しい snapshot に引き継ぎたい場合:

```powershell
python scripts\rebase_diffgr_state.py `
  --old out/work/r1.review.diffgr.json `
  --new out/work/r2.review.diffgr.json `
  --state out/state/r1.state.json `
  --output out/state/r2.state.json
```

用途:

- review state を別 repo / 別ファイルで管理していて、bundle 本体とは分離して rebase したい
- `rebase_reviews.py` の結果から state だけ取り出すのではなく、state JSON 単体で round2 に進めたい
...
- HTMLでコメント入力しても保存されない
  - 静的HTML方式は自動反映しない。`Download JSON` または `Download State` で取り出して手動反映する
  - 直接保存したいなら `serve_diffgr_report.py` を使う

- `Save State` が出ない
  - レポートに保存URLが埋め込まれていない
  - `serve_diffgr_report.py` 経由で開くか `--save-state-url` 付きで再出力する
...
- Textual の文字が小さい/大きい
  - 端末（Windows Terminal / VS Code Terminal 等）のフォント設定で調整する
  - 見た目の密度調整だけなら `=` / `-`（ui density）を使う

- HTML の文字が小さい/大きい
  - レポートのツールバー `A-` / `A+` / `A0` を使う（diff本文のフォントサイズ）
